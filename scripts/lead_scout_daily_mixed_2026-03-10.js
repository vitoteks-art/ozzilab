/* Lead Scout (Daily) — Firecrawl-only fallback (Mixed Regions)
   Target: 100 NEW leads/day, but limited by Firecrawl credits.

   NOTE: Apify Google Places step skipped because APIFY token/config not present in env.
...
function firecrawlSearch({ query, country, limit = 5 }) {
...
async function main() {
  ...
  // Hard cap scrapes due to limited Firecrawl credits.
  ...
  for (const row of queryPlan) {
    let out
    try {
      out = firecrawlSearch({ query: row.q, country: row.country, limit: 8 })
    } catch (e) {
      continue
    }
